...
bot.message(content: /(\d*)d(\d*)/i) do |msg|
  ...
  if rolls > 1000
    ...
    HyenaLogger.log("#{msg.author.display_name} (id: #{msg.author.id}) attempted to roll a #{rolls}d#{sides} but failed due to too many dice.")
  elsif sides > 1_000_000_000
    msg.respond("#{msg.author.display_name}, you can't roll dice with that many sides!")
    HyenaLogger.log("#{msg.author.display_name} (id: #{msg.author.id}) attempted to roll a #{rolls}d#{sides} but failed due to too many sided dice.")
  else
...
bot.message do |msg|
  HyenaLogger.log("#{msg.author.display_name} (id: #{msg.author.id}) said #{msg.content}")
end

bot.command(:exit, help_available: false, permission_level: 100) do |msg|
  HyenaLogger.log("#{msg.author.display_name} (id: #{msg.author.id}) issued command to exit.")
...
bot.playing do |event|
  if current_game == "D&D" && event.game
    ...
    HyenaLogger.log("#{event.user.username} (id: #{event.user.id}) was warned not to play #{event.game}")
  end
end
...
bot.sync
HyenaLogger.log("Initialization complete")
